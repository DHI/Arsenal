...
export const createVesselFeatureCollection = (
  ...
  const features = featureCollectionFeatures
    .map((f: Feature) => {
      const shipType = shipTypeIdToName(
        f.properties[vesselAttributeMapping.shipType] as number,
      );
      ...
      const toStern = f.properties[vesselAttributeMapping.toStern] as
        | number
        | null;
      const toPort = f.properties[vesselAttributeMapping.toPort] as
        | number
        | null;
      const toStarboard = f.properties[vesselAttributeMapping.toStarboard] as
        | number
        | null;
      const hullOverride = vesselAttributeMapping.hullOverride
        ? (f.properties[vesselAttributeMapping.hullOverride] as number[][])
        : null;
      const showInnerVesselLayout =
        vesselAttributeMapping.showInnerVesselLayout ?? null;
...
export const getVesselPolygons = (
  ...
  const layers: any[] = [];
  const vesselView = getVesselGeometry(
    vesselTypeName,
    color,
    hullOverride,
    showInnerVesselLayout,
  ...
  vesselView.geometry.forEach((vesselGeometry: VesselComponentView) => {
    const coordinates: number[][] = vesselGeometry.coordinates.map(
    ...
    const vesselDraft =
      draft > 0 ? draft : vesselView.defaultParams.draft * vesselScaleFactor;
    let elevation;

    if (vesselGeometry.isHull) {
      elevation =
        vesselView.defaultParams.hullHeight * vesselScaleFactor - vesselDraft;
    } else if (vesselGeometry.color === color.secondary) {
      elevation =
        vesselView.defaultParams.hullHeight * vesselScaleFactor * 1.2 -
        vesselDraft;
    } else if (vesselGeometry.color === color.tertiary) {
      elevation =
        vesselView.defaultParams.hullHeight * vesselScaleFactor * 1.4 -
        vesselDraft;
    } else if (vesselGeometry.color === color.primary) {
      elevation =
        vesselView.defaultParams.hullHeight * vesselScaleFactor * 1.3 -
        vesselDraft;
    } else {
      elevation =
        vesselView.defaultParams.hullHeight * vesselScaleFactor - vesselDraft;
